...
def get_augmenters(
    ...
    chain_augmenters_to_apply = (
        [
            ...
            Chain([Rotate(90), Flip(0) ]),
            Chain([Rotate(90), Flip(1) ]),
            Chain([Rotate(270), Flip(0) ]),
            Chain([Rotate(270), Flip(1) ]),
            Chain([Rotate(90), Transpose() ]),
            Chain([Rotate(270), Transpose() ]),
            Chain([Transpose(), Flip(0)]),
            Chain([Transpose(), Flip(1)]),

            Chain([Transpose(), Rotate(90)]),
            Chain([Transpose(), Rotate(270)]),
            Chain([Transpose(), Flip(0)]),
            Chain([Transpose(), Flip(1)]),
...
def process_task(
    ...
    if len(train) < 1000:
        ...
        leave_2_train_data = get_formatted_data(
            task, augmenters, formatter, tokenizer, leave_n=2, permute_n=3, seed=seed
        )

        leave_3_train_data = get_formatted_data(
            task, augmenters, formatter, tokenizer, leave_n=3, permute_n=3, seed=seed
        )


        train_redo = leave_1_train_data + leave_2_train_data + leave_3_train_data
       

    train = train + train_redo
  
    seen = set()
    filtered_data = []
    for item in train:
        hashable = dict_to_hashable(item)
    ...
    print(f'len(train_redo): {len(train_redo)}')
    print(f'number of duplicates after hash: {len(train) - len(filtered_data)}')
