...
run_prediction() {
    ...
    # Set up folders
    ttt_folder=${adapters_folder}/adapters_json_ep_${prev_epoch}_iter_-1
    tti_folder=${output_folder}/adapters_json_ep_${prev_epoch}_iter_-1
    mkdir -p $tti_folder

    echo "Epoch ${epoch}" | tee $output_log_folder/slurm_epoch_${prev_epoch}.log
